...
namespace ApplePushServices.Generic
{
    public class ApplePushServer<T> where T : new()
    {
        private ApplePushServicesInfo info;

        private Payload<T> PushPayload;
        ...
        public ApplePushServer(ApplePushServicesInfo _info, Payload<T> _payload, PushType type)
            : this()
        {
            this.info = _info;
        ...
        private void init()
        {
            ...
            this.DeviceToken = info.DevicesTokens;
            apnsClient.Connect(Public.HostServerAddress, Public.Port);
        }
        private void initMessage()
        {
            if (this.DeviceToken.Count > 0)
            {
                foreach (var item in this.DeviceToken)
                {
                    int identifier = 0;
                    ...
                    byte[] deviceToken = new byte[item.Length / 2];
                    for (int i = 0; i < deviceToken.Length; i++)
                        deviceToken[i] = byte.Parse(item.Substring(i * 2, 2), System.Globalization.NumberStyles.HexNumber);
                    ...
                    SendMessages.Add(new KeyValuePair<byte[], string>(this.BuildBufferFrom(notificationParts), item));
                }
            }
            else
            {
                this.err = new Exception("没有任何客户端数据，无法发送推送");
            }
        }
